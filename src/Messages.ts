...
export default class Messages {
    ...
    async fetchChats(filter = "visible") {
        try {
            ...
            return new ReturnObject(JSON.parse(await this.session.crypto.decryptAES(data.rows, this.session.sessionKey)).map((row: any) => {
    ...
    async replyToChat(uuid: string, content: string, receiver: string = "all") {
        try {
            const formData = new URLSearchParams();
